...
*/
    ...
    addToTail(head, 25.5);
    deleteAt(head, 2); // Deletes node at position 2 (which is 15.5)
    deleteList(head); // Deletes the entire list
...
void deleteAt(Node * & head, int pos) // Removed float paramter since the whole node is deleted, including value.
